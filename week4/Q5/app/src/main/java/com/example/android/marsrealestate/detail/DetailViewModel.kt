...
class DetailViewModel(marsProperty: MarsProperty, app: Application) : AndroidViewModel(app) {
    ...
    val selectProperty: LiveData<MarsProperty>
        get() = _selectProperty
    init {
        _selectProperty.value = marsProperty
    }

    val displayPropertyPrice = Transformations.map(selectProperty){
        app.applicationContext.getString(when(it.isRental){
            true -> R.string.display_price_monthly_rental
            false -> R.string.display_price
        }, it.price)
    }
    val displayPropertyType = Transformations.map(selectProperty) {
